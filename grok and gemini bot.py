...
def get_grok_response(prompt):
    try:
        # Create a chat completion request to Grok AI
...
def save_to_db(user_input, grok_response, gemini_response, conversation_id=None):
    if conversation_id:
...
def load_chat(chat_data):
    ...
    for message in chat_data["messages"]:
        ...
        chat_display.insert(tk.END, f"Grok: {message['grok_response']}\n", "grok")
...
def send_message():
    ...
    for i in range(interaction_limit):
        # Step 1: Get response from Grok AI
        grok_response = get_grok_response(current_input)
        chat_display.insert(tk.END, f"Grok: {grok_response}\n", "grok")
    ...
    if current_conversation_id:
        save_to_db(user_input, grok_response, gemini_response, conversation_id=current_conversation_id)
    else:
        current_conversation_id = save_to_db(user_input, grok_response, gemini_response)
...
chat_display.tag_config("grok", foreground="#85E085")
